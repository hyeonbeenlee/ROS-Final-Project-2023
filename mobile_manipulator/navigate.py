...
class ControlSuiteShell(cmd.Cmd):
    ...
    def do_navigate(self, arg):
        offset = 1
        trg_positions = [[3.2, -6.5],[-3, 4],[-8,-2.5]] ## position of hydrants in Gazebo
        for i, trg_pos in enumerate(trg_positions):
            print(f"Navigating to Hydrant {i+1}...") ## begin navigation
            scoutGoal = MoveBaseGoal()
            scoutGoal.target_pose.header.frame_id = 'map'
            scoutGoal.target_pose.pose.position.x = float(trg_pos[0] + offset)
            scoutGoal.target_pose.pose.position.y = float(trg_pos[1])
            ...
            print(f"Navigation completed. Manipulating arms...")
            ...
            self.group.stop()
            print(f"Manipulation completed. Releasing joints...")
